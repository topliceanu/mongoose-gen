...
var set = function (param) {
    return function (key, value) {
        if (!_.isFunction(value)) {
            throw new Error('expected type Function for '+value);
        }
        if (!_.isString(key)) {
            throw new Error('expected type String for '+key);
        }
        switch (param) {
            case 'validator':
...
var get = function (param, key) {
    ...
    if (!fn) {
        throw new Error('undefined '+param+' for name '+key);
    }
    return fn;
};
 ...
 */
var getSchema = function (descriptor, options) {
